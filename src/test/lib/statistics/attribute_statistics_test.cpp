...
TEST_F(AttributeStatisticsTest, SetStatisticsObject)
{
    ...
    attribute_statistics.set_statistics_object(std::make_shared<NullValueRatioStatistics>(0.2));
...
TEST_F(AttributeStatisticsTest, Scaled)
{
    ...
    attribute_statistics.range_filter =
        std::make_shared<RangeFilter<int32_t>>(std::vector{std::pair<int32_t, int32_t>(0, 100)});
    attribute_statistics.null_value_ratio = std::make_shared<NullValueRatioStatistics>(0.2);
    attribute_statistics.distinct_value_count = std::make_shared<DistinctValueCount>(12);

    const auto scaled_attribute_statistics =
        std::dynamic_pointer_cast<const AttributeStatistics<int32_t>>(attribute_statistics.scaled(0.5));
    ...
    EXPECT_DOUBLE_EQ(scaled_attribute_statistics->histogram->total_count(), 20);
    ...
    EXPECT_DOUBLE_EQ(scaled_attribute_statistics->null_value_ratio->ratio, 0.2);
...
TEST_F(AttributeStatisticsTest, ScaledWithIdentity)
{
    ...
    const auto null_value_ratio = std::make_shared<NullValueRatioStatistics>(0.2);
    ...
    const auto scaled_attribute_statistics =
        std::dynamic_pointer_cast<const AttributeStatistics<int32_t>>(attribute_statistics->scaled(1.0));
    ASSERT_TRUE(scaled_attribute_statistics);
...
TEST_F(AttributeStatisticsTest, Sliced)
{
    ...
    EXPECT_DOUBLE_EQ(sliced_attribute_statistics->histogram->total_count(), 20);
    ...
    EXPECT_DOUBLE_EQ(sliced_attribute_statistics->null_value_ratio->ratio, 0.0);
}
