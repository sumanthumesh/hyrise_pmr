 ...
 *
 * Implements the method with_iterators, which accepts a generic lambda (or similar) that expects a begin and end
 * iterator to the underlying data structure as parameters. Depending on this data structure, the generic lambda may be
 * instantiated for not one but many sets of iterators.
 * For example, the data structure might be accessed via a list of indices or might be nullable or non-nullable.
 *
 * For convenience, the class also implements the method for_each, which takes care of iterating over the elements and
 * accepts a generic lambda that expects a segment value as the parameter (use const auto& as the parameter
 * declaration).
 *
 *
 * A note on CRTP (curiously recurring template pattern):
 *
 * The iterables use the CRTP, i.e., the different iterable implementations derive from one of the base iterables and
 * pass themselves on as template arguments. The base class can then cast itself into the sub class and access its
 * methods. The pattern is used here to implicitly define the interface of a few templated methods. Templated methods
 * cannot be virtual in C++, so this could not be done with a normal abstract interface.
 * The advantage is that by looking at these two base iterables, it is clear what interface to expect from any iterable
 * and it also makes the otherwise implicit common interface of iterables more explicit.
 *
 *
 * Example Usage:
 *
 ...
 *
 * Extends the interface of SegmentIterable by two variants of with_iterators and for_each. In addition to the generic
 * lambda, these methods accept a PosList, which is used to filter the results.
 * The list is expected to use only that single chunk. When such a list is passed, the used iterators only iterate over
