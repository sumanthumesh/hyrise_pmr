...
template <typename T = ResolveDataTypeTag, EraseTypes erase_iterator_types = EraseTypes::OnlyInDebugBuild,
          typename Functor>
void segment_with_iterators(const AbstractSegment &abstract_segment, const Functor &functor)
{
    if constexpr (std::is_same_v<T, ResolveDataTypeTag>)
    {
        resolve_data_type(abstract_segment.data_type(), [&](const auto data_type_t)
                          {
      using ColumnDataType = typename decltype(data_type_t)::type;
      segment_with_iterators<ColumnDataType, erase_iterator_types>(abstract_segment, functor); });
    }
    else
    {
        if constexpr (HYRISE_DEBUG || erase_iterator_types == EraseTypes::Always)
        {
            const auto any_segment_iterable = create_any_segment_iterable<T>(abstract_segment);
...
template <typename T = ResolveDataTypeTag, EraseTypes erase_iterator_types = EraseTypes::OnlyInDebugBuild,
          typename Functor>
void segment_with_iterators_filtered(const AbstractSegment &abstract_segment,
                                     const std::shared_ptr<const AbstractPosList> &position_filter,
                                     const Functor &functor)
{
    if (!position_filter)
    {
        segment_with_iterators<T, erase_iterator_types>(abstract_segment, functor);
        return;
    }
    ...
    if constexpr (std::is_same_v<T, ResolveDataTypeTag>)
    {
        resolve_data_type(abstract_segment.data_type(), [&](const auto data_type_t)
                          {
      using ColumnDataType = typename decltype(data_type_t)::type;
      segment_with_iterators_filtered<ColumnDataType, erase_iterator_types>(abstract_segment, position_filter, functor); });
    }
    else
    {
        if constexpr (HYRISE_DEBUG || erase_iterator_types == EraseTypes::Always)
