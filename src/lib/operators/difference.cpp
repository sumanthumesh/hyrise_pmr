...
std::shared_ptr<const Table> Difference::_on_execute()
{
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count_left; ++chunk_id)
    {
        ...
        auto out_pos_list_map = std::unordered_map<std::shared_ptr<const AbstractPosList>, std::shared_ptr<RowIDPosList>>{};
        ...
        for (auto column_id = ColumnID{0}; column_id < column_count_left; ++column_id)
        {
            ...
            auto in_pos_list = std::shared_ptr<const AbstractPosList>{};
            ...
            auto &pos_list_out = out_pos_list_map[in_pos_list];
