...
query_ids = [7] #Queries to run

for dam_size in DAM_sizes:
    for table_fraction in table_fractions:
        for query_id in query_ids:
            #Run the mapping script with the given parameters
            cmd = f"python {MAPPING_SCRIPT} -p {HYRISE_ROOT}/myscripts/saved_data/sf100_hotness/acctr/hotness_{query_id}.json -s {HYRISE_ROOT}/myscripts/saved_data/column_sizes_sf100.dat -c {int(dam_size*table_fraction)},none -l 45,145 -o {os.path.abspath(f'scratch/sf100/access_counter/q7/map_d{dam_size}_f{int(table_fraction*100)}_q{query_id}.json')} --sql-out"
            ...
            # cmd = f"python {MAPPING_SCRIPT} -p {HYRISE_ROOT}/myscripts/saved_data/sf100_hotness/pqp/hotness_{query_id}.json -s {HYRISE_ROOT}/myscripts/saved_data/column_sizes_sf100.dat -c {int(dam_size*table_fraction)},none -l 45,145 -o {os.path.abspath(f'scratch/sf100/pqp/map_d{dam_size}_f{int(table_fraction*100)}_q{query_id}.json')} --sql-out"
            # cmd = f"python {MAPPING_SCRIPT} -p {HYRISE_ROOT}/myscripts/saved_data/sf100_hotness/lqp/hotness_{query_id}.json -s {HYRISE_ROOT}/myscripts/saved_data/column_sizes_sf100.dat -c {int(dam_size*table_fraction)},none -l 45,145 -o {os.path.abspath(f'scratch/sf100/lqp/map_d{dam_size}_f{int(table_fraction*100)}_q{query_id}.json')} --sql-out"
