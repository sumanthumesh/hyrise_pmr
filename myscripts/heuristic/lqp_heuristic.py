...
# Early-out payback ratio: when --early-out is set, aggregate benefit across
# all picked columns must exceed aggregate migration cost by at least this
# factor, otherwise the entire tableDAM is dropped (pin nothing).
...
def select_columns(plan: dict, col_sizes: dict, tabledam_budget_gb: float,
    ...
    If early_out=True, after packing the aggregate benefit across all picked
    columns must exceed aggregate migration cost by at least early_out_payback,
    otherwise everything is dropped and the whole budget rolls back to regDAM.
    """
